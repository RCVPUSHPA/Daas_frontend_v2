...
const ShowClusters = () => {
  ...
  let deleteCluster = (cluster_id) => {
    ...
    axiosInstance
      .delete(`${backendUrl}/v1/delete_cluster/${cluster_id}`, {
        headers: {
      ...
      .then((res) => {
        pc.setAvailableClusters([...res.data?.data?.clusters] || []);
  ...
  const updatePromxoxNodes = () => {
    ...
    axiosInstance
      ...
      .then(() => {
        ...
        return axiosInstance.get(`${backendUrl}/v1/clusters`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      })
      .then((res) => {
        pc.setAvailableClusters([...res.data?.data] || []);
        toast.success(res.data?.msg || "Nodes updated successfully" , { position: "top-right", autoClose: 3000 });
      })
      .catch((error) => {
