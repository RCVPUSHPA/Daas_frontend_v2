...
const TaskManagerPage = () => {
  ...
  useEffect(() => {
    axiosInstance
      .get(`${backendUrl}/v1/proxmox/proxmox_vm_info/${vmId}`)
      .then((res) => {
        setVmDetails(res.data.data);
      })
  ...
  const fetchAll = () => {
    ...
    // Processes
    axiosInstance.get(`${agentBackendUrl}/api/influxdb/fetch-background-processes`, {
      params: { 
    ...
    }).then((res) => setProcessData(res.data.data || []))
      .catch(() => setProcessData([]));

    // Host stats
    axiosInstance.get(`${agentBackendUrl}/api/influxdb/fetch-host-stats`, {
      params: { 
        bucket: config.bucket, 
        host: hostForApi,
        range_start: "-5m",
    ...
    }).then((res) => setHostStats(res.data || {}))
