...
const VamanitReports = (tokenParsed) => {
  ...
  useEffect(() => {
    ...
    setLoader(true); 
    axiosInstance
      .get(`${backendUrl}/v1/guacamole/vamanit_allusers/${start}/${end}`, {
        headers: {
  ...
  const fetchCompanyDetails = (reportType) => {
    axiosInstance
      .get(`${backendUrl}/v1/guacamole/reports/${reportType}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((response) => {
        const companyData = response.data.data.find(
  ...
  const fetchSessionReports = () => {
    ...
    axiosInstance
      ...
      .then((response) => {
        setSessionReports(response.data.data);
        setPrint(true);
  ...
  const fetchDayReports = () => {
    ...
    axiosInstance
      ...
      .then((response) => {
        setDayReports(response.data.data);
        setPrint(true);
        setShowSessionReports(false);
  ...
  const fetchConsolidateReports = () => {
    ...
    axiosInstance
      ...
      .then((response) => {
        ...
        setConsolidateReports(response.data.data);
        setPrint(true);
        setShowSessionReports(false);
        setShowDayReports(false);
