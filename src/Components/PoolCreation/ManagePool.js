...
const ManagePool = (props) => {
  ...
  const fetchAssignedUsers = (machineIdentifier, machineId) => {
    setUsersLoading(true);
    setSelectedVm(machineId);
    setSelectedVmIdentifier(machineIdentifier);
    axiosInstance
      .get(`${backendUrl}/v1/machine/users/${machineId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        const assignedUsersData = res.data?.data || [];
      ...
      try {
        ...
        results.forEach((result, idx) => {
          if (result.status === "fulfilled" && result.value.data?.data) {
            detailsMap[vmIds[idx]] = result.value.data?.data;
          }
        });
  ...
  let fetchMachineDetails = async (vm_id) => {
    ...
    try {
      const res = await axiosInstance.get(
        `${backendUrl}/v1/proxmox/proxmox_vm_info/${vm_id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      setSelectedVmDetails(res.data?.data);
    } catch (err) {
  ...
  let deleteAssignedUser = (user) => {
    ...
    axiosInstance
      .delete(
        `${backendUrl}/v1/delete_user_from_machine/${selectedVmIdentifier}/${user}`,
        {
          headers: {
    ...
    const fetchVMs = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `${backendUrl}/v1/pool/machines/${selectedPoolDetails.id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        if (Array.isArray(response.data?.data)) {
          setVmAvailable(response.data?.data);
        } else {
          setVmAvailable([]);
        }
      } catch (err) {
  ...
  let entitleUser = (usr) => {
    ...
    axiosInstance
      .post(
        `${backendUrl}/v1/add_user_to_machine/${selectedVmIdentifier}/${usr}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  let deletePool = () => {
    ...
    axiosInstance
      .request({
        method: "DELETE",
        url: `${backendUrl}/v1/delete_pool/${selectedPoolDetails.id}`,
  ...
  useEffect(() => {
    axiosInstance
      .get(`${backendUrl}/v1/guacamole/list_users`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  }, [assignedUsers, token, backendUrl]);
  ...
  const handleReboot = async () => {
    ...
    try {
      await axiosInstance.post(
        ...
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { vmid: selectedVmObj.vm_id, pool_id: poolId },
        }
      );
  ...
  const handleRebuildVM = async (item) => {
    ...
    try {
      const response = await axiosInstance.post(
        ...
        { email: userEmail }, // Sending email in the body
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { vmid: item.vm_id, pool_id: poolId },
        }
      );
