...
export const GetAllIpmiLists = async (token) => {
  try {
    const response = await axiosInstance.get(`${getEnv("BACKEND_URL")}/v1/ipmi/get_all_ipmi_servers`, {
...
export const createIpmiServer = async (token, payload) => {
  try {
    const response = await axiosInstance.post(`${getEnv("BACKEND_URL")}/v1/ipmi/add_ipmi_server`, payload, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
export const deleteIpmiServer = async (token, ipmi_id, userEmail) => {
  try {
    await axiosInstance.delete(`${getEnv("BACKEND_URL")}/v1/ipmi/delete_ipmi_server/${ipmi_id}`, {
      headers: {
...
export const getIpmiServerById = async (token, id) => {
  try {
    const response = await axiosInstance.get(`${getEnv("BACKEND_URL")}/v1/ipmi/get_ipmi_server/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data?.data;
  } catch (error) {
...
export const updateIpmiServer = async (token, id, payload) => {
  try {
    await axiosInstance.put(`${getEnv("BACKEND_URL")}/v1/ipmi/update_ipmi_server/${id}`, payload, {
      headers: { Authorization: `Bearer ${token}` },
    });
